...
class OdmpyLibbyTests(BaseTestCase):
    ...
    def test_mock_libby_download_audiobook_odm(self):
        ...
        self.assertEqual(audio_file.tags[Tag.Title].text[0], "Ceremonies For Christmas")
        self.assertEqual(audio_file.tags[Tag.Album].text[0], "Ceremonies For Christmas")
        self.assertEqual(audio_file.tags[Tag.Artist].text[0], "Robert Herrick")
        self.assertEqual(audio_file.tags[Tag.AlbumArtist].text[0], "Robert Herrick")
        self.assertEqual(audio_file.tags[Tag.Conductor].text[0], "LibriVox Volunteers")
    ...
    def test_mock_libby_download_audiobook_direct(self):
        ...
        for part_file in part_files:
            ...
            self.assertEqual(audio_file.tags[Tag.Title].text[0], loan["title"])
            self.assertEqual(audio_file.tags[Tag.Album].text[0], loan["title"])
            self.assertEqual(
                audio_file.tags[Tag.Artist].text[0], loan["firstCreatorName"]
            )
            self.assertEqual(
                audio_file.tags[Tag.AlbumArtist].text[0], loan["firstCreatorName"]
            )
            self.assertEqual(
            ...
            self.assertEqual(
                audio_file.tags[Tag.Publisher].text[0], loan["publisherAccount"]["name"]
            )
            self.assertEqual(
                audio_file.tags[f"{Tag.UserDefinedText}:ISBN"].text[0],
    ...
    def test_mock_libby_download_audiobook_direct_merge(self):
        ...
        self.assertEqual(audio_file.tags[Tag.Title].text[0], loan["title"])
        self.assertEqual(audio_file.tags[Tag.Album].text[0], loan["title"])
        self.assertEqual(audio_file.tags[Tag.Artist].text[0], loan["firstCreatorName"])
        self.assertEqual(
            audio_file.tags[Tag.AlbumArtist].text[0], loan["firstCreatorName"]
        )
        self.assertEqual(
        ...
        self.assertEqual(
            audio_file.tags[f"{Tag.UserDefinedText}:ISBN"],
            [f for f in loan["formats"] if f.get("isbn")][0]["isbn"],
        )
        self.assertTrue(audio_file.tags[f"{Tag.TableOfContents}:toc"])
        chapters = [t for t in audio_file.tags.getall(Tag.Chapter)]
